...
object MyService {
  ...
  implicit def jsonParser[F[_], A : TypeTag : ClassTag: ResultPrimitiveMetadata]: StringParser[F, A] = new StringParser[F, A] with FailureResponseOps[F] {
    def metadata: ResultPrimitiveMetadata[A] = ResultPrimitiveMetadata[A]
