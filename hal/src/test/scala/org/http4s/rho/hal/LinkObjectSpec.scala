...
class LinkObjectSpec extends Specification {
  ...
  "LinkObject" should {
    ...
    "have a type property optionally" in {
      new LinkObject(
        "/link",
        `type` = Some("application/json")).`type`.get must be equalTo "application/json"
    }
    "have a deprecation property optionally" in {
      new LinkObject(
        "/link",
        deprecation =
          Some("http://more/info/about/deprecated")).deprecation.get must be equalTo "http://more/info/about/deprecated"
    }
    ...
    "have a title property optionally" in {
      new LinkObject(
        "/link",
        title =
          Some("The case for hyperlinks in APIs")).title.get must be equalTo "The case for hyperlinks in APIs"
    }
    "have a hreflang property optionally" in {
      new LinkObject(
        "/link",
        hreflang = Some("/href/lang")).hreflang.get must be equalTo "/href/lang"
    }
