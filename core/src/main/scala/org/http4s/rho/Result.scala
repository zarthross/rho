...
trait ResultSyntaxInstances[F[_]] {
  implicit class ResultSyntax[R >: Result.TopResult[F] <: Result.BaseResult[F]](r: R) {
    ...
    def withTrailerHeaders(trailerHeaders: F[Headers]): Self =
      Result(resp.withTrailerHeaders(trailerHeaders))
    ...
    def withContentType(contentType: `Content-Type`): Self =
      Result(resp.withContentType(contentType))
    ...
    def withContentTypeOption(contentTypeO: Option[`Content-Type`]): Self =
      Result(resp.withContentTypeOption(contentTypeO))
    ...
    def addCookie(name: String, content: String, expires: Option[HttpDate] = None): Self =
      Result(resp.addCookie(name, content, expires))
