...
sealed trait ResultResponse[F[_], +T] extends RouteResult[F, T] {
  def map[T2](f: T => T2): ResultResponse[F, T2] =
    this match {
      case SuccessResponse(v) => SuccessResponse(f(v))
      case e @ FailureResponse(_) => e
    }

  def flatMap[T2](f: T => ResultResponse[F, T2]): ResultResponse[F, T2] =
    this match {
      case SuccessResponse(v) => f(v)
      case e @ FailureResponse(_) => e
    }

  def orElse[T2 >: T](other: => ResultResponse[F, T2]): ResultResponse[F, T2] =
    this match {
...
object FailureResponse {
  ...
  def pure[F[_]](response: => F[Response[F]]): FailureResponse[F] =
    FailureResponse(new ResponseReason(response))

  /** Construct a [[FailureResponse]] using the provided thunk. */
  def result[F[_]](result: => F[BaseResult[F]])(implicit F: Functor[F]): FailureResponse[F] =
    pure(F.map(result)(_.resp))
}
