...
object ResultMatcher extends ResultMatcherOps {
  ...
  trait MaybeWritableOps {
    ...
    implicit def maybeIsWritable[F[_], T](implicit
        t: WeakTypeTag[T],
        w: EntityEncoder[F, T] = null): MaybeWritable.Aux[F, T] =
      new MaybeWritable[T] {
  ...
  implicit def optionMatcher[F[_], R](implicit
      o: WeakTypeTag[R],
      w: EntityEncoder[F, R]): ResultMatcher[F, Option[R]] =
    new ResultMatcher[F, Option[R]] with ResponseGeneratorInstances[F] {
  ...
  implicit def writableMatcher[F[_], R](implicit
      o: WeakTypeTag[R],
      w: EntityEncoder[F, R]): ResultMatcher[F, R] =
    new ResultMatcher[F, R] with ResponseGeneratorInstances[F] {
  ...
  implicit def responseMatcher[F[_]]: ResultMatcher[F, Response[F]] =
    new ResultMatcher[F, Response[F]] {
      override def encodings: Set[MediaType] = Set.empty
...
trait ResultMatcherOps {
  implicit def fMatcher[F[_], R](implicit r: ResultMatcher[F, R]): ResultMatcher[F, F[R]] =
