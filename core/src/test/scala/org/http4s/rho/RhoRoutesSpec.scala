...
class RhoRoutesSpec extends Specification with RequestRunner {
  ...
  "RhoRoutes execution" should {
    ...
    "Execute a route with a competing query" in {
      ...
      (checkOk(req1) should_== "route5")
        .and(checkOk(req2) should_== "route6_bar")
        .and(checkOk(req3) should_== "route7")
    }
    ...
    "Execute a variadic route" in {
      ...
      (checkOk(req1) should_== "route8_")
        .and(checkOk(req2) should_== "route8_one")
        .and(checkOk(req3) should_== "route8_one/two")
    }

    "Perform path 'or' logic" in {
      val req1 = Get("/or1")
      val req2 = Get("/or2")
      (checkOk(req1) should_== "route9").and(checkOk(req2) should_== "route9")
    }

    "Work with options" in {
      val req1 = Get("/options")
      val req2 = Get("/options?foo=bar")
      (checkOk(req1) should_== "None").and(checkOk(req2) should_== "bar")
    }
    ...
    "Work with collections" in {
      ...
      (checkOk(req1) should_== "")
        .and(checkOk(req2) should_== "bar")
        .and(checkOk(req3) should_== "1 2")
    }
    ...
    "Attempt to evaluate params in order" in {
      ...
      getBody(
        service(req1).value.unsafeRunSync().getOrElse(Response.notFound).body) must_== "Int: 0"

      val req2 = Request[IO](Method.GET, Uri(path = "/foo").+?("bar", "s"))
      getBody(
        service(req2).value.unsafeRunSync().getOrElse(Response.notFound).body) must_== "String: s"
    ...
    "Fail to match more specific routes defined after a less specific route" in {
      ...
      val req1 = Request[IO](Method.GET, Uri(path = "/foo").+?("bar", "0"))
      getBody(
        service(req1).value.unsafeRunSync().getOrElse(Response.notFound).body) must_== "String: 0"
    ...
    "Match an empty Option over a bare route" in {
      ...
      getBody(
        service(req1).value.unsafeRunSync().getOrElse(Response.notFound).body) must_== "String: s"
